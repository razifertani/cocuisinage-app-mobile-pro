...
class Planning {
  ...
  Planning.fromJson(Map<String, dynamic> json) {
    ...
    shouldFinishAt = json['should_finish_at'] != null ? DateFormat.Hm().parse(json['should_finish_at']) : null;
    startedAt = json['started_at'] != null ? DateFormat.Hm().parse(json['started_at']) : null;
    finishedAt = json['finished_at'] != null ? DateFormat.Hm().parse(json['finished_at']) : null;
    status = json['status'];
  ...
  int getDelayInMinutes() {
    if (this.status == IN_PROGRESS) {
      ...
      return this.startedAt!.difference(this.shouldStartAt).inMinutes + this.shouldFinishAt!.difference(this.finishedAt!).inMinutes;
    } else {
      return 0;
    }
  }
