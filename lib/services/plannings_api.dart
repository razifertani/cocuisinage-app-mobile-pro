...
Future<Either<Exception, String>> addPlanningWS(
    {required int professionalId,
    required DateTime day,
    DateTime? shouldStartAt,
    DateTime? startedAt,
    ...
    final response = await http.post(
      ...
      body: {
        ...
        if (shouldStartAt != null)
          'should_start_at': DateFormat.Hm().format(shouldStartAt),
        if (startedAt != null) 'started_at': DateFormat.Hm().format(startedAt),
        if (shouldFinishAt != null)
          'should_finish_at': DateFormat.Hm().format(shouldFinishAt),
...
Future<Either<Exception, String>> updatePlanningWS(
    {required Planning planning, bool? monthly}) async {
  try {
    final response = await http.post(
      Uri.parse(
...
Future<Either<Exception, String>> updatePlanningTimeWS(
    {required int id,
    required DateTime startedAt,
    DateTime? finishedAt,
    DateTime? shouldFinishAt}) async {
  try {
    final response = await http.post(
      Uri.parse(
      ...
      body: {
        ...
        if (finishedAt != null)
          'finished_at': DateFormat.Hm().format(finishedAt),
        if (shouldFinishAt != null)
          'should_finish_at': DateFormat.Hm().format(shouldFinishAt),
      },
    );
