...
Future<Either<Exception, String>> addTaskWS({required int collegueID, required int planningID, required String taskName}) async {
  try {
...
Future<Either<Exception, String>> updateTaskWS({required int collegueID, required int id, int? status, String? comment, File? image}) async {
  try {
    http.Response response;

    if (image != null) {
      var request = http.MultipartRequest('POST', Uri.parse('${Globals.baseUrl}/tasks/${id.toString()}'))
        ..headers.addAll({
          HttpHeaders.acceptHeader: 'application/json',
          HttpHeaders.authorizationHeader: 'Bearer ${Globals.token}',
